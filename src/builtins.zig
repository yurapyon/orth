...
fn displayValue(t: *Thread, value: Value) void {
    switch (value) {
        ...
        .Rc_Ptr => |ptr| {
            const ty = t.vm.type_table.items[ptr.type_id].ty.Rc;
            if (ptr.is_weak) {
...
fn areValuesEqual(a: Value, b: Value) bool {
    return if (@as(@TagType(Value), a) == b) switch (a) {
        ...
        .Rc_Ptr => |ptr| ptr.type_id == b.Rc_Ptr.type_id and
            ptr.rc == b.Rc_Ptr.rc and
...
fn areValuesEquivalent(t: *Thread, a: Value, b: Value) bool {
    return if (@as(@TagType(Value), a) == b) switch (a) {
        ...
        .Rc_Ptr => |ptr| t.vm.type_table.items[ptr.type_id].ty.Rc.equivalent_fn(t, ptr, b),
